...
```{r}
table_of_power <- table_of_power %>% mutate(doy_1 = lubridate::yday(ymd_hms(substr(table_of_power$utc_date, 1, 19))))
...
```{r}
table_of_power_merged_1 <- table_of_power_merged %>% group_by(interact_id, doy_1) %>% 
  filter(doy_1 >= doy_start) %>% filter(doy_1 <= doy_end) 

table_of_power_merged_1 <- table_of_power_merged_1 %>% mutate(n_points = 1)
#adding a point to each row to allow for sum of points per day to filter again

table_of_power_merged_1 <- table_of_power_merged_1 %>% group_by(interact_id, doy_1) %>%
   dplyr::mutate(day_points = sum(n_points))


tabyl(table_of_power_merged_1$day_points)
table_of_power_merged_2 <- table_of_power_merged_1 %>% filter(day_points > 125)
#filtering entire data frame for points greater than 7500 seconds or 125 minutes  
```

## New table with PA by day
```{r}
table_of_power_merged_2 <- table_of_power_merged_2 %>% group_by(interact_id, doy_1) %>%
...
write_csv(table_of_power_merged_2, "table_of_power_merged_2.csv")



table_of_power_merged_3 <- table_of_power_merged_2 %>% group_by(interact_id, doy_1) %>% 
  summarise(day_sedentary = first(day_sedentary), 
            day_light = first(day_light), 
            day_moderate = first(day_moderate), 
            day_vigorous = first(day_vigorous))

table_of_power_merged_3 <- table_of_power_merged_3 %>% group_by(interact_id) %>%
  mutate(total_sedentary = sum(day_sedentary), 
         total_light = sum(day_light), 
         total_moderate = sum(day_moderate), 
         total_vigorous = sum(day_vigorous))

table_of_power_merged_3 <- table_of_power_merged_3 %>% group_by(interact_id) %>%
  mutate(total_pa = total_light + total_moderate + total_vigorous)

write_csv(table_of_power_merged_3, "table_of_power_merged_3.csv")
```

## Small data frame table
```{r}
table_of_power_merged_4 <- table_of_power_merged_3 %>% dplyr::select("interact_id", "total_sedentary", "total_light", "total_moderate", "total_vigorous", "total_pa") %>% 
  distinct(interact_id, .keep_all = TRUE)

write_csv(table_of_power_merged_4, "table_of_power_merged_4.csv")
```
...
power_victoria_merged <- full_join(table_of_power_merged_2, victoria_small_merged_1, by = "interact_id") #update once I get new table of power. 
write_csv(power_victoria_merged, "power_victoria_merged.csv")
summary_power_victoria_merged <- full_join(table_of_power_merged_4, victoria_small_merged_1, by = "interact_id") #update once you get new table of power
write_csv(summary_power_victoria_merged, "summary_power_victoria_merged.csv")
```
