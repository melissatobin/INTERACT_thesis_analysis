...
#bike_lanes <- st_read(dsn ="./Bike_Lanes",layer = "Bike_Lanes.shp") #potentially just pandora
#this didn't work
...
gps_join_1 <- lapply(all_gps_list_project, function(i) {
  i %>%
    group_by(interact_id) %>%
    summarise(n = n()) %>%
    as_tibble()
}) #THIS WORKS!!!!
#fwrite(gps_join_1, "gps_join_1.csv", sep = ",")
...
gps_join_2 <- lapply(filter_gps_in_buffer, function(i) {
  i %>%
    group_by(interact_id) %>% summarise(n = n()) %>% as_tibble()
})
...
gps_wear_time_overall <- lapply(all_gps_list_project,function(i) {
  i %>% group_by(interact_id) %>% 
...
gps_wear_time_by_day_unique_time_stamps <- lapply(all_gps_list_project,function(i) {
  i %>% group_by(interact_id) %>% 
    mutate(doy = yday(utcdate)#,
           ) %>% 
    group_by(doy) %>%
...
save(gps_wear_time_by_day_unique_time_stamps, file = "gps_wear_time_by_day_unique_time_stamps.rda")
#this gives the time of day that participants wore as well as total time per day

load("all_gps_list_project.rda")

gps_wear_time_by_day_min_max_time_stamps <- lapply(all_gps_list_project, function(i) {
  i %>% group_by(interact_id) %>% 
    mutate(doy = yday(utcdate)
           ) %>% 
    group_by(doy) %>%
    summarise(interact_id = interact_id,
...
#save(gps_wear_time_by_day_min_max_time_stamps, file = "gps_wear_time_by_day_min_max_time_stamps.rda")
...
fwrite(wear_time_stamps, "wear_time_stamps.csv", sep = ",", showProgress = T)
