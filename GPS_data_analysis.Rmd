...
join_gps_to_buffer <- lapply(gps_time_stamps_filtered, function(i) {
  i %>%
...
```{r}
gps_join_1 <- lapply(gps_time_stamps_filtered, function(i) {
  i %>%
...
gps_join_all_select <-
  select_(gps_join_all, "interact_id.y", "n.x", "n.y")
...
write_csv(gps_join_all_percent, "gps_join_all_percent.csv")
...
gps_wear_time_by_day_unique_time_stamps <- lapply(gps_time_stamps_filtered,function(i) {
  i %>% group_by(interact_id.y) %>% 
    mutate(doy = yday(utcdate)#,
           ) %>% 
    group_by(doy) %>%
...
save(gps_wear_time_by_day_unique_time_stamps, file = "gps_wear_time_by_day_unique_time_stamps_updated.rda")
#this gives the time of day that participants wore as well as total time per day



gps_wear_time_by_day_min_max_time_stamps <- lapply(gps_time_stamps_filtered, function(i) {
  i %>% group_by(interact_id.y) %>% 
    mutate(doy = yday(utcdate)
           ) %>% 
    group_by(doy)%>%
