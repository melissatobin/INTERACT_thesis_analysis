...
## GPS Wear Time 
```{r}
summary(victoria_small_merged$total_hours)
sd(victoria_small_merged$total_hours)

summary(victoria_small_merged$number_days)
sd(victoria_small_merged$number_days)

summary(victoria_small_merged$average_hours)
sd(victoria_small_merged$average_hours)
```
...
victoria_small_merged <- victoria_small_merged %>% mutate(ethnicity_updated = case_when(
 ...
 ethnicity == "Latin American" ~ "Other",
...
victoria_small_merged_1 <- filter(victoria_small_merged, gender_updated == "Women" or "Men") #fix this
```

## Final Fit Regression Models 
```{r}
victoria_small_merged <- victoria_small_merged %>% 
...
victoria_small_merged %>% finalfit(dependent, explanatory) -> t1
knitr::kable(t1, row.names = FALSE, align = c("l", "l", "r", "r", "r")) 



lm_1 <- lm(total_pa_met_formula ~ percent + gender_updated + age_categories + ethnicity_updated + income_2 + mean_temp_date + total_precip_date, data = victoria_small_merged)
summary(lm_1)

victoria_small_merged$lm_fitted <- fitted(lm_1)
