...
```{r}
power_victoria_merged_updated <- power_victoria_merged_updated %>% 
                              mutate_at(vars(income_2, age_categories, ethnicity_updated), as.factor)
...
power_victoria_merged_updated %>% finalfit(dependent, explanatory) -> t3
knitr::kable(t3, row.names = FALSE, align = c("l", "l", "r", "r", "r")) 
```

## Making plots for regression - accelerometer data
```{r}
pa_regression_1 <- lm(summary_count ~ percent,  data = power_victoria_merged_updated)

summary(pa_regression_1)
lm.beta(pa_regression_1) ##This isn't working. 
confint(pa_regression_1)

ggplot(power_victoria_merged_updated, aes(y = summary_count, x = percent)) + 
  geom_point() +
  geom_smooth(method = "lm")
```
...
```{r}
summary(power_victoria_merged_updated$summary_count)
sd(power_victoria_merged_updated$summary_count, na.rm = TRUE)

ggplot(power_victoria_merged_updated) + 
                      geom_histogram(aes(summary_count))
  
```

## Negative binomial regression with just percent and summary count
```{r}
nb_regression_1 <- glm.nb(summary_count ~ percent, data = power_victoria_merged_updated)
...
```{r}
nb_regression_2 <- glm.nb(summary_count ~ percent + gender_updated + age_categories + ethnicity_updated + income_2 + mean_temp_date + total_precip_date, data = power_victoria_merged_updated)
