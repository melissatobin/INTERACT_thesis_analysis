...
```{r}
nb_regression_1 <- glm.nb(summary_count ~ percent, data = power_victoria_accel_filtered)
summary(nb_regression_1)
```

## Negative binomial regression with all predictors - updated
```{r}
nb_regression_2 <- glm.nb(summary_count ~ percent + gender_updated + age_categories + ethnicity_updated + income_2 + mean_temp_date + total_precip_date, data = power_victoria_accel_filtered)
...
## Filtered exposure - updating regression models
```{r}
...
lm_2 <- lm(total_pa_met_formula ~ percent + gender_updated + age_categories + ethnicity_updated + income_2 + mean_temp_date + total_precip_date, data = victoria_exposure_filtered)
summary(lm_2)
confint(lm_2)
```
